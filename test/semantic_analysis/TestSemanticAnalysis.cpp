...
TEST_F(SemanticAnalysis, MissingReturnStatement) {
    auto ast = create_ast("VAR density;"
                "BEGIN\n"
               "density := 10\n"
               "END.");
    EXPECT_FALSE(ast);
...
TEST_F(SemanticAnalysis, AssignmentToConstant) {
    auto ast = create_ast("CONST density = 1;\n"
                "BEGIN\n"
               "density := 10;\n"
               "RETURN density\n"
               "END.");
...
TEST_F(SemanticAnalysis, RedeclarationIdentifier) {
    auto ast = create_ast("PARAM d, d;"
                "BEGIN\n"
               "RETURN density\n"
               "END.");
    ASSERT_FALSE(ast);

    ast = create_ast("PARAM d;\n"
               "CONST d = 2;"
               "BEGIN\n"
               "RETURN density\n"
               "END.");
...
TEST_F(SemanticAnalysis, SymbolTableCapturesAllDeclarations) {
    ...
    auto ast = create_ast("PARAM width, height, depth;\n"
               "VAR volume, some;\n"
               "CONST density = 2400;\n"
                "BEGIN RETURN 0 END.");
    ASSERT_TRUE(ast);
    auto symbolTable = ast->getSymbolTable();
...
TEST_F(SemanticAnalysis, ConstantsHaveValueSet) {
    auto ast = create_ast("CONST density = 2400, a = 10;\n"
