...
struct terminal_node : public node_base {
    ...
    explicit terminal_node(lexer::token token) : node_base(TEXT_NODE, token.get_code_reference()), token(token) {}
...
struct literal_node : public non_terminal_node {
    ...
    explicit literal_node(node_ptr token, source_management::SourceFragment source) : non_terminal_node(LITERAL, source, std::move(token)) {}
    void accept(parse_tree_visitor& visitor) const override;
...
struct identifier_node : public non_terminal_node {
    ...
    identifier_node(node_ptr token, source_management::SourceFragment source) : non_terminal_node(IDENTIFIER, source, std::move(token)) {}
    void accept(parse_tree_visitor& visitor) const override;
...
struct declarator_list_node : public non_terminal_node {
    ...
    declarator_list_node(source_management::SourceFragment fragment, std::unique_ptr<identifier_node> identifier,
                         std::vector<std::pair<
                             ...
                             otherIdentifiers) : non_terminal_node(DECLARATOR_LIST, fragment, std::move(identifier)) {
...
struct init_declarator_list_node : public non_terminal_node {
    ...
    init_declarator_list_node(source_management::SourceFragment fragment,
                              ...
                              std::vector<std::pair<
                                  std::unique_ptr<terminal_node>,
                                  std::unique_ptr<init_declarator_node>>>
                                  otherIdentifiers) : non_terminal_node(INIT_DECLARATOR_LIST, fragment, std::move(identifier)) {
        for (auto& entry : otherIdentifiers) {
            add_child(std::move(entry.first), std::move(entry.second));
        }
    }
